    ...
    <div id="comments" class="comments-area">
        ...
        <?php if (have_comments()) : ?>
            <h2 class="comments-title text-2xl font-bold text-gray-900 mb-8">
                ...
                if ($comment_count === 1) {
                    printf(_x('One comment on &ldquo;%s&rdquo;', 'comments title', 'modern-theme'), get_the_title());
                } else {
                    printf(
                        _nx(
            ...
            <?php the_comments_navigation(array(
                'prev_text' => __('← Older Comments', 'modern-theme'),
                'next_text' => __('Newer Comments →', 'modern-theme'),
            ...
            <?php if (!comments_open()) : ?>
                <p class="no-comments text-gray-600 italic mt-8">
                    <?php _e('Comments are closed.', 'modern-theme'); ?>
                </p>
            <?php endif; ?>
        ...
        <article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
                    ...
                    <div class="comment-date-time text-sm text-gray-600">
                        <a href="<?php echo esc_url(get_comment_link($comment, $args)); ?>" class="hover:text-primary-600 transition-colors duration-200">
                            <time datetime="<?php comment_time('c'); ?>">
                                <?php printf(_x('%1$s at %2$s', '1: date, 2: time', 'modern-theme'), get_comment_date('', $comment), get_comment_time()); ?>
                            </time>
                        </a>
                        <?php edit_comment_link(__('(Edit)', 'modern-theme'), '<span class="edit-link ml-2">', '</span>'); ?>
                    </div>
                </div>
            </footer>
            ...
            <?php if ('0' == $comment->comment_approved) : ?>
                <div class="comment-awaiting-moderation bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-2 rounded mb-4">
                    <?php _e('Your comment is awaiting moderation.', 'modern-theme'); ?>
                </div>
            <?php endif; ?>
